...
class FrameRenderer:
    ...
    def render(
        ...
        *,
        ...
        raw_tracks: Optional[List[RawTrack]] = None,  # <-- NOVÉ
        ...
        if self.show_stats:
            y = 26
            _put_text(frame_bgr, f"frame {frame_idx}/{total_frames if total_frames is not None else '?'}", (10, y), scale=0.6, thick=1)
            y += 26
            if fps:
